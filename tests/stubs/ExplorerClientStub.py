...
from tfchain.types.ConditionTypes import ConditionBaseClass
from tfchain.types.CryptoTypes import PublicKey
from tfchain.types.PrimitiveTypes import Hash
from tfchain.types.ThreeBot import BotName
from tfchain.TFChainClient import ThreeBotRecord
from tfchain.internal.jsutils import generateXByteID, json_dumps


class TFChainExplorerGetClientStub:
    def __init__(self):
        self._blocks = {}
        self._hashes = {}
    ...
    def explorer_get(self, endpoint):
        ...
        mint_condition_at_template = re.compile(
            r'^.*/explorer/mintcondition/(\d+)$')
        ...
        mint_condition_latest_template = re.compile(
            r'^.*/explorer/mintcondition$')
    ...
    def explorer_post(self, endpoint, data):
        ...
        if match:
            transactionid = str(
                Hash(value=generateXByteID(Hash.SIZE)))
    ...
    def block_get(self, height):
        ...
        if not height in self._blocks:
            raise tfchain.errors.ExplorerNoContent("no content found for block {}".format(
                height), endpoint="/explorer/blocks/{}".format(height))
    ...
    def block_add(self, height, resp, force=False):
        ...
        if not force and height in self._blocks:
            raise KeyError(
                "{} already exists in explorer blocks".format(height))
    ...
    def hash_get(self, hash):
        ...
        if not hash in self._hashes:
            raise tfchain.errors.ExplorerNoContent("no content found for hash {}".format(
                hash), endpoint="/explorer/hashes/{}".format(str(hash)))
    ...
    def threebot_record_add(self, record, force=False):
        ...
        if not force and record.identifier in self._threebot_records:
            raise KeyError(
                "3Bot {} already exists in explorer threebot records".format(record.identifier))
    ...
    def mint_condition_add(self, condition, height, force=False):
        ...
        if height in self._mint_conditions and not force:
            raise KeyError("{} already exists in explorer mint conditions on height {}".format(
                str(condition.unlockhash), height))
        if not isinstance(condition, ConditionBaseClass):
            raise TypeError(
                "condition is expected to be a subtype of ConditionBaseClass: {} is an invalid type".format(type(condition)))
    ...
    def mint_condition_get(self, height, endpoint):
        ...
        if len(self._mint_conditions) == 0:
            raise tfchain.errors.ExplorerServerError(
                "no mint conditions defined", endpoint=endpoint)
