...
class OutputLock:
    ...
    def __init__(self, value=None, current_timestamp=None):
        ...
        if value is None:
            ...
            if value[0] == "+":
                ...
                offset = duration.fromString(value[1:])
                self._value = current_timestamp + offset
            else:
                # interpret string as a datetime
                self._value = jsdatetime.fromString(value)
    ...
    def __str__(self):
        if self.is_timestamp:
            return jstime.epoch2HRDateTime(self._value)
...
class AtomicSwapSecret(BinaryData):
    ...
    @classmethod
    def random(cls):
        return cls(value=generateXByteID(AtomicSwapSecret.SIZE))
